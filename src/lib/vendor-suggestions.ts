 ...
 */
export async function suggestVendors(params: VendorSuggestionParams): Promise<VendorSuggestion[]> {
  const { category, eventCity, eventState, budgetTotalCents, maxResults = 6 } = params
  ...
  return scoredVendors.sort((a, b) => b.score - a.score).slice(0, maxResults)
}

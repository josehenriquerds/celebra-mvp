...
import {
  Home, Users, CheckSquare, UserCog, Utensils, TrendingUp, Settings,
  Menu, X, Gift, ClipboardCheck, FileBarChart, Clock, Filter, ChevronLeft, ChevronRight
} from 'lucide-react';
    ...
    <TooltipProvider delayDuration={150}>
      <div className="min-h-dvh bg-[#FAF7F4] text-[var(--ink)]">
        ...
        >
          ...
          <div className="absolute -left-6 top-6 bottom-6 w-24 rounded-3xl bg-pastel-peach-200/70 blur-[1px]" />
          <motion.div
            layout
            style={{ width }}
            className={cn(
              "relative h-[calc(100dvh-3rem)] my-6 mr-4 rounded-3xl",
              "bg-white/80 backdrop-blur-sm border border-black/5 shadow-elevation-2"
            )}
          >
            {/* header do rail */}
            <div className="flex items-center justify-between px-3 pt-3 pb-2">
              {!collapsed && (
                <motion.div
                  ...
                  className="flex-1"
                >
                  <h1 className="text-xl font-heading font-bold text-celebre-brand">Celebre</h1>
                  <p className="text-xs text-celebre-muted mt-0.5">Gestão de Eventos</p>
                </motion.div>
              )}
              <motion.button
                ...
                className={cn(
                  ...
                  "bg-white/70 hover:bg-white shadow-sm border border-black/5",
            ...
            {/* nav */}
            <nav aria-label="Navegação Principal" className="mt-2 overflow-y-auto overflow-x-hidden h-[calc(100vh-12rem)] custom-scrollbar">
            ...
            <div className="absolute bottom-3 left-0 right-0 px-3">
              <motion.div
                className={cn(
                  ...
                  "px-3 py-2.5 flex items-center gap-3 transition-all cursor-pointer"
                )}
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
              >
                <div className="size-9 rounded-full bg-gradient-to-br from-pastel-mint-200 to-pastel-mint-300 flex items-center justify-center">
                  <span className="text-sm font-bold text-celebre-brand">J</span>
                </div>
                {!collapsed && (
                  <motion.div
                  ...
                  >
                    <p className="font-semibold leading-none truncate text-celebre-ink">Jack</p>
                    <p className="text-celebre-muted text-xs mt-0.5">Organizador</p>
                  </motion.div>
                )}
              </motion.div>
            </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-lg supports-[backdrop-filter]:bg-white/80 border-t border-border shadow-elevation-2 lg:hidden">
            ...
            {navigation.slice(0, 5).map((item) => {
              ...
              return (
                <Link
                  ...
                  href={href}
                  className={cn(
                    "flex flex-col items-center gap-1 px-2 py-2.5 text-[10px] font-medium transition-all relative",
                ...
                >
                  {active && (
                    <motion.div
                      ...
                      className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-celebre-brand rounded-full"
...
function SidebarItem({
  item, href, active, collapsed,
}: { item: Item; href: string; active: boolean; collapsed: boolean }) {
    ...
    <Link
      ...
      className={cn(
        ...
        active
          ...
          : "text-[var(--ink)]/60 hover:text-[var(--ink)] hover:bg-white/50"
      )}
    >

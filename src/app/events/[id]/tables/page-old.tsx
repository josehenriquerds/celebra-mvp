...
function DroppableSeat({ seat, tableId, zoom }: { seat: Seat; tableId: string; zoom: number }) {
  ...
  return (
    <div
      ...
      className={`absolute flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 touch-none select-none items-center justify-center rounded-full ${isOver ? 'ring-celebre-brand/60 bg-celebre-accent/20 ring-2' : ''} ${isDragging ? 'opacity-70' : ''}`}
  ...
  return (
    ...
    >
      ...
      <div
        {...listeners}
        {...attributes}
        className="bg-celebre-brand absolute left-1/2 top-2 -translate-x-1/2 cursor-move touch-none select-none rounded-full p-1 text-white opacity-0 transition-opacity group-hover:opacity-100"
        title="Arrastar mesa"
      >
      ...
      {/* Ações */}
      <div className="absolute -top-8 right-0 flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
        <button
          onClick={(e) => {
            e.stopPropagation()
            onEdit(table)
        ...
        <div className="max-w-full px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="w-16 text-center text-sm text-celebre-muted">
                {Math.round(zoom * 100)}%
              </span>
              <Button
                variant="outline"
        ...
        <div className="flex h-[calc(100vh-120px)]">
          ...
          <div className="w-80 overflow-y-auto border-r bg-white">
            <div className="border-b p-4">
              ...
              <p className="mt-1 text-xs text-celebre-muted">Arraste para as mesas à direita</p>
            </div>

            {/* Criar mesa */}
            <div className="bg-celebre-accent/20 border-b p-4">
              <h3 className="mb-3 text-sm font-medium">Nova Mesa</h3>
            ...
            <UnassignedDropzone>
              {data.unassigned.length === 0 ? (
                <div className="py-8 text-center text-celebre-muted">
                  ...
                  <p className="mt-1 text-xs">Arraste aqui para desalocar</p>
                </div>
              ) : (
          ...
          <div className="flex-1 select-none overflow-auto bg-gray-50">
            <div
              id="planner-canvas"
              className="relative"
              style={{
              ...
              {data.tables.length === 0 ? (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center text-celebre-muted">
                    ...
                    <p className="mt-2 text-sm">Use o formulário à esquerda para criar mesas</p>
                  </div>
                </div>
              ) : (
                data.tables.map((t) => (
          ...
          <DragOverlay>
            ...
            {activeId && activeType === 'table' && activeTable && (
              <div className="border-celebre-brand flex h-24 w-24 items-center justify-center rounded-full border-4 bg-white shadow-lg">
                <div className="text-center">
                  <div className="text-celebre-brand text-sm font-bold">{activeTable.label}</div>
                </div>
              </div>
            )}
          </DragOverlay>

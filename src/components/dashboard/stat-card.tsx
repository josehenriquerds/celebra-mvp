    ...
    >
      <Card className="hover:shadow-celebre-lg transition-shadow duration-300">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium text-celebre-muted">
            {title}
          </CardTitle>
          {Icon && (
            <Icon className="h-4 w-4 text-celebre-muted" />
          )}
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold text-celebre-ink">{value}</div>
          {description && (
            <p className="text-xs text-celebre-muted mt-1">{description}</p>
          )}
          {trend && (

...
## Autentica��o de Hosts

A autentica��o de anfitri�es agora � baseada em NextAuth com sess�o JWT e sele��o de evento.

### Fluxo de login
- /login: coleta o telefone e verifica a exist�ncia do anfitri�o.
- /login/create-password: permite definir a primeira senha quando ainda n�o existe.
- /login/password: autentica com telefone + senha.
- /login/select-event: lista os eventos vinculados e persiste o evento atual na sess�o.
...
Todos os endpoints exigem cabe�alho x-csrf-token alinhado com o cookie 
ext-auth.csrf-token e aplicam rate limit (5 req/min) + bloqueio ap�s 5 tentativas inv�lidas.

### Vari�veis de ambiente
- NEXTAUTH_SECRET: obrigat�rio para assinar os tokens.
- AUTH_DEFAULT_COUNTRY (opcional, padr�o BR): pa�s usado ao normalizar telefones.
- BCRYPT_COST (opcional, m�nimo 10): custo de hashing de senha.
...
O seed cria tr�s anfitri�es de exemplo. A senha padr�o � Celebre123! para contas que j� t�m hash gerado.

### Testes
- 
pm run test executa a su�te de unit + integra��o (Vitest).
- Playwright pode ser usado para E2E (
pm run test:e2e) ap�s configurar o ambiente local.
